...
A component described as data is typed and linted before anything runs. A cross-stack reference resolves by name instead of getting scraped out of another stack with `jq`. A bad reference or a missing output is a build error at author time rather than a failure halfway through the deploy.
...
The hundredth component stays as cheap as the first. That is what the shared apply verb bought on Kubernetes. Off the cluster you recover it by describing the component as data and compiling the release from it.
